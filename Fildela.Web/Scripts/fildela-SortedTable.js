...
(function () {
    ...
    $("body").mousedown(function (e) {
        if (e.which == 3 && $(e.target).parent().hasClass('account-table-tr') || $(e.target).hasClass('account-files-preview-link')) {
            SortedTable.DropdownOpen = true;
            prevClick = e.which;
        }
        else {
            if (prevClick != 3)
                SortedTable.DropdownOpen = false;
...
SortedTable.prototype = {
    ...
    callBodyClick: function (e) {
        ...
        var elm = st.findParent(elm, 'tr');
        //EDIT
        if (!$(e.target).hasClass('account-files-preview-link') && !$(e.target).hasClass('account-files-name')) {
            if ($(e.target).parent().hasClass('selrow') && $(e.target).parent().hasClass('account-table-tr') && e.shiftKey)
                return false;
            else {
                if (!$(e.target).parent().hasClass('account-table-tr'))
                    return false;
                if (e.shiftKey && st.allowMultiple) {
                    st.selectRange(elm);
                    //EDIT
                    SetTableMenuDisplay();
                }
                    ...
                    if (st.selected(elm)) {
                        if (st.allowDeselect) {
                            //EDIT
                            if (!SortedTable.DropdownOpen) {
                                if (!$(e.target).hasClass('account-files-edit-name-input') && !$(e.target).hasClass('account-files-edit-name-submit') && $('.account-files-edit-name-container:visible').length != 1 && !$(e.target).hasClass('account-files-edit-name-btn-text')) {
                                    SortedTable.DropdownOpen = false;
                                    st.deselect(elm);
                                    SetTableMenuDisplay();
                                }
